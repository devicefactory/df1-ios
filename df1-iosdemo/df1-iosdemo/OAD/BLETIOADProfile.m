...
-(void) configureProfile
{
    ...
    [DF1LibUtil setNotificationForCharacteristic:self.d.p sCBUUID:sUUID cCBUUID:cUUID enable:YES];
    unsigned char data = 0x00;
    [DF1LibUtil writeCharacteristic:self.d.p sCBUUID:sUUID cCBUUID:cUUID data:[NSData dataWithBytes:&data length:1]];
...
-(void) actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    switch (actionSheet.tag) {
        case 0: {
            switch(buttonIndex) {
                case 0: {
                    UIActionSheet *selectInternalFirmwareSheet = [[UIActionSheet alloc]initWithTitle:@"Select Firmware image" delegate:self cancelButtonTitle:@"Cancel" destructiveButtonTitle:nil otherButtonTitles:
                            @"DF1_rangefix.bin",@"DF1_hires_v0.1.bin",@"DF1_varfreq.bin", nil];
        ...
        case 1: {
            switch (buttonIndex) {
                case 0: {
                    ...
                    [path appendString:@"/"] ;
                    [path appendString:@"DF1_rangefix.bin"];
                ...
                case 1: {
                    NSMutableString *path= [[NSMutableString  alloc] initWithString: [[NSBundle mainBundle] resourcePath]];
                    [path appendString:@"/"] ;
                    [path appendString:@"DF1_hires_v0.1.bin"];
                    [self validateImage:path];
                    break;
                }
                case 2: {
                    NSMutableString *path= [[NSMutableString  alloc] initWithString: [[NSBundle mainBundle] resourcePath]];
                    [path appendString:@"/"] ;
                    [path appendString:@"DF1_varfreq.bin"];
                    [self validateImage:path];
                    break;
                }
                default:
...
-(void) uploadImage:(NSString *)filename
{
    ...
    [DF1LibUtil writeCharacteristic:self.d.p sCBUUID:sUUID cCBUUID:cUUID data:[NSData dataWithBytes:requestData length:OAD_IMG_HDR_SIZE + 2 + 2]];
...
-(void) programmingTimerTick:(NSTimer *)timer
{
    ...
    for (int ii = 0; ii < 4; ii++) {
        ...
        [DF1LibUtil writeNoResponseCharacteristic:self.d.p sCBUUID:sUUID cCBUUID:cUUID data:[NSData dataWithBytes:requestData length:2 + OAD_BLOCK_SIZE]];
